              ...
              try {
                ...
                if (cfg.provider === 'openai' && cfg.openaiKey) {
                  ...
                  const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${cfg.geminiModel || 'gemini-1.5-flash'}:generateContent?key=${cfg.geminiKey}`, {
                    ...
                    body: JSON.stringify({
                      contents: [{ parts: [{ text: `Provide ONLY the raw code completion for the following snippet (do not wrap in markdown blocks, do not explain). Start exactly where the snippet leaves off:\n\n${textUntilPosition}` }] }],
                      generationConfig: { maxOutputTokens: 24, stopSequences: ['\n'] }
                    })
                  });
                  if (response.ok) {
                    const data = await response.json();
